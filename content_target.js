...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  ...
  if (message.type === "interceptedProductAPI" && message.site === "target" && !processedUrls.has(message.url)) {
...
async function fetchProductDetails(url) {
  ...
  try {
    ...
    let ingredientDesc = '';
    if (data && data.product && data.product.children && data.product.children.length > 0) {
      ingredientDesc = data.product.children[0].item?.enrichment?.nutrition_facts?.ingredients || '';
    } else if (data && data.data && data.data.product) {
      ingredientDesc = data.data.product.item?.enrichment?.nutrition_facts?.ingredients || '';
    }
    ...
    if (ingredientDesc) {
      ...
      // Split the cleaned description into an array of ingredients
      const fullIngredientList = cleanIngredientDesc.split(/\s*[,\n]\s*/).map(ingredient => ingredient.trim().toLowerCase());
      
      const foundHarmfulIngredients = window.harmfulIngredients.filter(ingredient => 
        fullIngredientList.includes(ingredient.toLowerCase())
      );
      ...
      console.log('Full Ingredient List:', fullIngredientList.join(', '));
    } else {
